...
/*! \fn  int parser(node * temp, char * a, int size)
    \brief Generates a rooted binary parse tree, with its root at \a temp, from string \a prefixstr of length \a len where \a prefixstr is a proposition in prefix form.  
